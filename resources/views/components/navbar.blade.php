<nav class="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm">
    <div class="max-w-full mx-auto" style="padding-left: 40px; padding-right: 40px;">
        <div class="flex justify-between items-center h-16" style="gap: 60px;">
            <!-- Logo -->
            <a href="{{ route('home') }}" class="flex-shrink-0 hover:opacity-80 transition duration-300">
            ...
            <div class="flex items-center flex-1" style="gap: 80px;">
                <a href="{{ route('home') }}" class="text-gray-700 hover:text-blue-600 font-medium text-sm whitespace-nowrap transition duration-300">
                    Home
                </a>
                <a href="{{ route('jobs.search') }}" class="text-gray-700 hover:text-blue-600 font-medium text-sm whitespace-nowrap transition duration-300">
                    Browse Jobs
                </a>
                <a href="{{ route('companies.browse') }}" class="text-gray-700 hover:text-blue-600 font-medium text-sm whitespace-nowrap transition duration-300">
                    Companies
                </a>
                <a href="{{ route('community.index') }}" class="text-gray-700 hover:text-blue-600 font-medium text-sm whitespace-nowrap transition duration-300">
                    Community
                </a>
            </div>
            ...
            <div class="flex items-center gap-4 flex-shrink-0">
                    ...
                    <div class="relative inline-block">
                        <button onclick="toggleProfileDropdown(event)" class="flex items-center gap-2 px-2 py-1 rounded-full hover:bg-gray-100 transition duration-300">
                            @if(auth()->user()->profile_picture)
                                <img src="{{ asset('storage/' . auth()->user()->profile_picture) }}" alt="{{ auth()->user()->name }}" class="w-9 h-9 rounded-full object-cover shadow-md hover:shadow-lg transition duration-300">
                            @else
                        ...
                        <div id="profileDropdown" class="hidden absolute right-0 mt-3 w-72 bg-white rounded-2xl shadow-2xl border border-gray-200 z-50 overflow-hidden">
                            ...
                            <!-- Menu Items -->
                            <div class="py-3">
                                @if(auth()->user()->isApplicant() || (auth()->user()->role === 'jobseeker'))
                ...
                @else
                    <a href="{{ route('login') }}" class="px-4 py-2 text-gray-700 hover:text-gray-900 font-medium text-sm transition duration-300 whitespace-nowrap">
                        Sign In
                    </a>
                @endauth
                
                <!-- Employer Site Link - Always Visible -->
                <a href="{{ route('employer.landing') }}" class="px-4 py-2 text-blue-600 hover:text-blue-700 font-medium text-sm transition duration-300 whitespace-nowrap">
                    Employer site
                </a>
            </div>
