        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-lg border border-gray-200 p-6 sticky top-24">
                    ...
                    @auth
                        <a href="{{ route('community.create') }}" class="w-full px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg transition duration-200 flex items-center justify-center gap-2 mb-6">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="lg:col-span-3">
                ...
                @if($threads->count() > 0)
                    <div class="space-y-4">
                        @foreach($threads as $thread)
                            <a href="{{ route('community.show', $thread) }}" class="block group">
                                <div class="bg-white rounded-lg border border-gray-200 p-6 hover:border-gray-300 hover:shadow-md transition duration-200">
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <div class="mb-2">
                                                <p class="text-xs font-semibold text-gray-600 uppercase tracking-wide">{{ $thread->company->name }}</p>
                                            </div>
                                            <h3 class="text-lg font-bold text-gray-900 group-hover:text-blue-600 transition mb-2">{{ $thread->title }}</h3>
                                            <p class="text-gray-700 text-sm line-clamp-2 mb-3">{{ $thread->last_message_preview }}</p>
                                            ...
                                            <div class="flex items-center gap-6 text-sm text-gray-600">
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <span>{{ $thread->messages->count() }}</span>
                                                </div>
                                                <div class="text-gray-500">{{ $thread->last_activity_at->diffForHumans() }}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                @else
                    ...
                    <div class="bg-white rounded-lg border border-gray-200 p-12 text-center">
                        ...
                        @auth
                            <a href="{{ route('community.create') }}" class="inline-block px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg transition duration-200">
                                Create First Thread
                            </a>
                        @else
